...
# stations D3 and D4 are outside the DSA so SIMS, DETAW and CalSIMETAW don't provide estimates.
# remove these stations for these models from the dataframe prior to plotting.

fieldstations_data_2015_filter <- fieldstations_data_2015 %>% filter(!(Station %in% c("D3", "D4") & model %in% c("sims", "calsimetaw", "detaw")))

# remove D4
fieldstations_data_2015_filter <- fieldstations_data_2015_filter %>% filter(!Station %in% c("D4"))
...
p <- ggplot(d, aes(x=Station, y=ET_mean, color=model, group=measure_type))+
  geom_point(size=3.5)+
  ...
  geom_text(aes(x=1.35,y=eto), size=4, nudge_y = 0.2, colour="#000000", label='ETo')+
  geom_text(aes(x=1.35,y=field_mean_et, group=measure_type), size=4, nudge_y = 0.2, colour="#990000", label='ETa')+
