...
write.csv(df_dsa_legal, 'data/dsa_etrf/dsa_etrf.csv', row.names = FALSE)
...
# make into one dataframe  
df_regions <- ldply(listofdfs, data.frame)  

####################################################################

saveRDS(df_regions, file="data/delta_regions/delta_regions.rds")
write.csv(df_regions, 'data/delta_regions/delta_regions.csv', row.names = FALSE)

```


## Refernce ETO for DSA
...
####################################################################
# files shoulds be named "data/model-region-wy.json"
list_of_files <- list.files(path="data/eto", pattern=".geojson", full.names=TRUE)
...
# make into one dataframe  
df_eto_rough <- ldply(listofdfs, data.frame)  

df_eto_rough <- rename(df_eto_rough, value = X..i..)

# separate
s <- df_eto_rough %>% separate(month, c("month", "stat"))

df_eto <- s %>% spread(stat, value)
####################################################################

saveRDS(df_eto, file="data/eto/eto.rds")
write.csv(df_eto, 'data/eto/eto.csv', row.names = FALSE)
