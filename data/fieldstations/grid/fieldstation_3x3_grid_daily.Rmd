...
This Rmarkdown notebook is from post-processing the daily data for the 3x3 grid around each of the field station.
...
rs_drop <- grid_daily %>% select(-c(id))
...
tab <- read.csv("fieldstation_grid_daily_detaw_calsimetaw.csv")
...
write.csv(m, 'fieldstation_grid_daily.csv', row.names = FALSE)
saveRDS(m, file="fieldstation_grid_daily.rds")
