...
selected_model_name <- "DisALEXI" # uppercase
...
Plots show two scenarios for DisALEXI - orange is the `OUTPUT` results while black is the `OUTPUT_CORR` results
```{r plot_line_15, fig.width=7, fig.height=4, echo=FALSE}

subset_data <- data %>% filter(model %in% c(selected_model, 'disalexi_co'), region == "dsa")
for(i in 1:length(crops_2_gen)){
  id <- lookup_cropid(crops_2_gen[i])
  model_palette = c("disalexi"="#e69f00", "disalexi_co"="#000000")
