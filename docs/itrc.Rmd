...
Plots show two scenarios for ITRC. Green is the `OR` results while black is the `CO` results
```{r plot_line_15, fig.width=7, fig.height=4, echo=FALSE}

subset_data <- data %>% filter(model %in% c(selected_model, 'itrc_co'), region == "dsa")
for(i in 1:length(crops_2_gen)){
  id <- lookup_cropid(crops_2_gen[i])
  model_palette = c("itrc"="#009e73", "itrc_co"="#000000")
