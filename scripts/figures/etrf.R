...
etrf <- function(data, crop_name){
  ...
  j_ETrF_crops <- j_ETrF %>% filter(cropname == crop_name) %>% mutate(date=date_from_wy_month(wateryear, month)) # add a date field by combining month and water year
