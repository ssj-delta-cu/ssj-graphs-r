...
af_crops_w_others$model <- factor(af_crops_w_others$model, levels=c("calsimetaw", "detaw", "disalexi", "itrc", "sims", "ucdmetric", "ucdpt"))
levels(af_crops_w_others$model) <- c("CalSIMETAW", "DETAW", "DisALEXI", "ITRC", "SIMS", "UCD-METRIC", "UCD-PT")
af_crops_w_others$cropname <- factor(af_crops_w_others$cropname, levels=rev(c("Alfalfa", "Corn", "Pasture", "Other")))


p <- ggplot(af_crops_w_others, aes(x=factor(wateryear), y=sum_af, fill=cropname))+geom_bar(stat = "identity") + 
  #scale_x_discrete(labels=c("CalSIMETAW", "DETAW", "DisALEXI", "ITRC", "SIMS", "UCD-METRIC", "UCD-PT"))+
  #scale_x_discrete(labels=c("2015", "2016"))+
  theme_bw() + 
